...
public class JavaClient : ISdkClient
{
    ...
    public void Initialize(string serverUrl)
    {
        try
        {
            ...
            // Find JAR file
            _jarPath = FindConductorJar();
            if (string.IsNullOrEmpty(_jarPath))
            {
                throw new InvalidOperationException("Conductor JAR file not found. Please ensure the JAR files are properly downloaded.");
            }
    ...
    private string FindConductorJar()
    {
        var possiblePaths = new[]
        {
            Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "conductor-client.jar"),
            Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "lib", "conductor-client.jar"),
            Path.Combine(Directory.GetCurrentDirectory(), "conductor-client.jar"),
            Path.Combine(Directory.GetCurrentDirectory(), "lib", "conductor-client.jar"),
            Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", "..", "..", "..", "SdkTestAutomation.Sdk", "bin", "Debug", "net8.0", "lib", "conductor-client.jar")
        };
    ...
    private void TestJavaConnection()
    {
        try
        {
            var result = ExecuteJavaCall("event", "get-event", new { });
